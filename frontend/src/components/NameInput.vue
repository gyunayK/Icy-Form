<template>
    <div class="relative w-full">
        <label for="technology-name" class="text-sm text-white text-left">
            Technology name
        </label>
        <div class="relative">
            <input type="text" id="technology-name" :value="modelValue" @blur="handleInputChange($event.target.value)"
            ...
            <font-awesome-icon v-if="inputError === 'User Not Exist'" :icon="['fas', 'circle-check']"
                class="absolute right-3 top-1/2 transform -translate-y-1/2 text-green" />
            <font-awesome-icon v-if="inputError === 'User Exist'" :icon="['fas', 'xmark']"
                class="absolute right-3 top-1/2 transform -translate-y-1/2 text-red" />
        </div>
        <p v-if="errorMessage" class="text-sm" :class="{
    ...
    methods: {
        async handleInputChange(value) {
            ...
            try {
                ...
                this.inputError = true; // Default to error state
            }
        }
    }
};
